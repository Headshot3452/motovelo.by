<?php
    ...
    class PagerWidget extends BsPager
    {
        protected function createPageLinks()
        {
            ...
            if ($this->lastestPageLabel === null)
            {
                $this->lastestPageLabel = Yii::t('yii', '&gt;&gt;');
            }
...
//                $links[] = array('label' => ' ... ',);
    ...
    // next page
            ...
            if ((!$this->hideFirstAndLast and ($pageCount != $currentPage + 1)))
            {
                $links[] = $this->createPageLink($this->lastestPageLabel, $pageCount - 1, $currentPage == $pageCount - 1, false);
            }
