    ...
    class FeedbackController extends FrontendController
    {
        ...
        public function actionContacts()
        {
            if(isset($_POST['ApplicationForm']))
            {
                ...
                $model->sort = $sort;
                $model->status = Feedback::STATUS_NEW;
                $model->time = time();
                $model->parent_id = 1;
                $model->ask = $_POST['ApplicationForm']['text'];

                if($model->validate())
                {
                    $model->save();

                    $applicationModel = new ApplicationForm();
                    $applicationModel->attributes = $_POST['ApplicationForm'];
                ...
                $feedback_answer = new FeedbackAnswers();
                $feedback_answer->language_id = $language;
                $feedback_answer->feedback_id = $model->id;
                $feedback_answer->settings_feedback_id = 1;
                $feedback_answer->value = $_POST['ApplicationForm']['name'];
                $feedback_answer->save(false);
                ...
                $feedback_answer->feedback_id = $model->id;
                $feedback_answer->settings_feedback_id = 2;
                $feedback_answer->value = $_POST['ApplicationForm']['phone'];
                $feedback_answer->save(false);
                ...
                $feedback_answer->feedback_id = $model->id;
                $feedback_answer->settings_feedback_id = 3;
                $feedback_answer->value = $_POST['ApplicationForm']['email'];
                $feedback_answer->save(false);
            }
            $this->redirect($this->createUrl('site/contacts'));
        }
